...
async def list_paths(token: str, db: AsyncSession = Depends(get_db)):
...
async def get_path(path_id: str, token: str, db: AsyncSession = Depends(get_db)):
    await _get_current_user_id(token)
    result = await db.execute(
        select(LearningPath)
...
async def list_room_lessons(room_id: str, token: str, db: AsyncSession = Depends(get_db)):
...
async def get_lesson_detail(lesson_id: str, token: str, db: AsyncSession = Depends(get_db)):
    user_id = await _get_current_user_id(token)
    ...
    # Prev / next lesson in the same room
    result = await db.execute(
...
async def start_question(question_id: str, token: str, db: AsyncSession = Depends(get_db)):
    user_id = await _get_current_user_id(token)
...
async def submit_answer(
    ...
    # Check answer — with placeholder support
        ...
        if _hash_answer(template) in (expected_hash.split("|") if expected_hash else []):
            ...
            if _hash_answer(payload.answer) == _hash_answer(resolved):
                ...
                if result.scalar_one_or_none() is None:
                    db.add(UserQuestionProgress(user_id=user_id, question_id=question.id))
                    await db.commit()
...
async def _maybe_mark_lesson_complete(db: AsyncSession, user_id, lesson_id):
    ...
    # Get all question ids for this lesson
...
async def get_dashboard_stats(token: str, db: AsyncSession = Depends(get_db)):
    ...
    total_score = lessons_completed * 10 + levels_completed * 20 + questions_answered * 2
    ...
    result = await db.execute(
        select(UserProgress.completed_at, Level.title)
    ...
    level_events = [
        RecentActivityItem(type="level", title=title, completed_at=completed_at.isoformat())
        for completed_at, title in result.all()
    ]

    recent = sorted(lesson_events + level_events, key=lambda x: x.completed_at, reverse=True)[:5]
